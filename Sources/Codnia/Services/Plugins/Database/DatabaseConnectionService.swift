...
public final class DatabaseConnectionService: ObservableObject {
    ...
    public func removeConnection(_ config: ConnectionConfig) {
        ...
        DatabaseKeychainHelper.delete(account: config.id)
    ...
    public func connect(_ config: ConnectionConfig, password: String, database: String? = nil) async {
        ...
        do {
            ...
            DatabaseKeychainHelper.save(account: config.id, password: password)
        } catch {
            sshTunnelService.stopTunnel(configID: config.id)
    ...
    public func password(for configID: String) -> String? {
        DatabaseKeychainHelper.get(account: configID)
    }
    ...
    public func execute(configID: String, sql: String, page: Int = 0, pageSize: Int = 100, orderBy: String? = nil) async -> QueryPageResult {
        ...
        let timeout = config.queryTimeout

        do {
            let result = try await withThrowingTaskGroup(of: QueryPageResult.self) { group in
                group.addTask {
                    try await provider.execute(handle: handle, query: sql, page: page, pageSize: pageSize, orderBy: orderBy)
                }
                group.addTask {
                    try await Task.sleep(nanoseconds: UInt64(timeout) * 1_000_000_000)
                    throw DatabaseConnectionError.timeout(timeout)
                }
            ...
            return result
        } catch let error as DatabaseConnectionError {
            return QueryPageResult(
                columns: [], rows: [], totalCount: 0,
                page: page, pageSize: pageSize,
    ...
    public func exportSchemaDDL(configID: String, schema: String) async -> String {
        let allTables = await fetchTables(configID: configID, schema: schema)
        var lines: [String] = []
        lines.append("--")
        lines.append("-- Schema dump for \"\(schema)\"")
        lines.append("-- Generated by Codnia")
        lines.append("--")
        lines.append("")

        let tables = allTables.filter { $0.tableType == .table }
        for t in tables {
            let ddl = await fetchTableDDL(configID: configID, table: TableID(schema: schema, table: t.name))
            if !ddl.isEmpty {
                lines.append("-- Table: \(t.name)")
                lines.append(ddl)
                if !ddl.hasSuffix(";") { lines[lines.count - 1] += ";" }
                lines.append("")
            }
        }

        let views = allTables.filter { $0.tableType == .view || $0.tableType == .materializedView }
        for v in views {
            let source = await fetchRoutineSource(configID: configID, schema: schema, name: v.name, type: .view)
            if !source.isEmpty {
                lines.append("-- View: \(v.name)")
                lines.append(source)
                if !source.hasSuffix(";") { lines[lines.count - 1] += ";" }
                lines.append("")
            }
        }

        let funcs = await fetchFunctions(configID: configID, schema: schema)
        for f in funcs {
            let source = await fetchRoutineSource(configID: configID, schema: schema, name: f.name, type: .function)
            if !source.isEmpty {
                lines.append("-- Function: \(f.name)")
                lines.append(source)
                if !source.hasSuffix(";") { lines[lines.count - 1] += ";" }
                lines.append("")
            }
        ...
        let procs = await fetchProcedures(configID: configID, schema: schema)
        for p in procs {
            let source = await fetchRoutineSource(configID: configID, schema: schema, name: p.name, type: .procedure)
            if !source.isEmpty {
                lines.append("-- Procedure: \(p.name)")
                lines.append(source)
                if !source.hasSuffix(";") { lines[lines.count - 1] += ";" }
                lines.append("")
            }
    ...
    public func fetchTriggers(configID: String, schema: String) async -> [TriggerInfo] {
        guard let (provider, handle) = ddlProvider(for: configID) else { return [] }
        return (try? await provider.fetchTriggers(handle: handle, schema: schema)) ?? []
    }

    public func dropTrigger(configID: String, schema: String, trigger: String, table: String) async throws {
        guard let (provider, handle) = ddlProvider(for: configID) else { throw DatabaseConnectionError.notConnected }
        try await provider.dropTrigger(handle: handle, schema: schema, trigger: trigger, table: table)
        schemaVersion += 1
    }

    // MARK: - Sequences

    public func fetchSequences(configID: String, schema: String) async -> [SequenceInfo] {
        guard let (provider, handle) = ddlProvider(for: configID) else { return [] }
        return (try? await provider.fetchSequences(handle: handle, schema: schema)) ?? []
    }

    public func dropSequence(configID: String, schema: String, sequence: String) async throws {
        guard let (provider, handle) = ddlProvider(for: configID) else { throw DatabaseConnectionError.notConnected }
        try await provider.dropSequence(handle: handle, schema: schema, sequence: sequence)
        schemaVersion += 1
    }

    // MARK: - Constraints

    public func fetchConstraints(configID: String, table: TableID) async -> [ConstraintInfo] {
        guard let (provider, handle) = ddlProvider(for: configID) else { return [] }
        return (try? await provider.fetchConstraints(handle: handle, table: table)) ?? []
    }

    public func dropConstraint(configID: String, table: TableID, constraint: String) async throws {
        guard let (provider, handle) = ddlProvider(for: configID) else { throw DatabaseConnectionError.notConnected }
        try await provider.dropConstraint(handle: handle, table: table, constraint: constraint)
        schemaVersion += 1
    }

    public func addForeignKey(configID: String, table: TableID, name: String, columns: [String], refTable: TableID, refColumns: [String], onDelete: String?, onUpdate: String?) async throws {
        guard let (provider, handle) = ddlProvider(for: configID) else { throw DatabaseConnectionError.notConnected }
        try await provider.addForeignKey(handle: handle, table: table, name: name, columns: columns, refTable: refTable, refColumns: refColumns, onDelete: onDelete, onUpdate: onUpdate)
        schemaVersion += 1
    }

    // MARK: - Table Properties

    public func fetchTableStats(configID: String, table: TableID) async -> TableStats {
        guard let (provider, handle) = ddlProvider(for: configID) else { return TableStats() }
        return (try? await provider.fetchTableStats(handle: handle, table: table)) ?? TableStats()
    }

    public func fetchTableProperties(configID: String, table: TableID) async -> TableProperties {
        let tableInfo = TableInfo(schema: table.schema, name: table.table)
        async let stats = fetchTableStats(configID: configID, table: table)
        async let columns = fetchColumns(configID: configID, table: table)
        async let indexes = fetchIndexes(configID: configID, table: table)
        async let constraints = fetchConstraints(configID: configID, table: table)
        let allTriggers = await fetchTriggers(configID: configID, schema: table.schema)
        let tableTriggers = allTriggers.filter { $0.table == table.table }
        async let deps = fetchDependencies(configID: configID, schema: table.schema, table: table.table)
        async let ddl = fetchTableDDL(configID: configID, table: table)
        async let pks = fetchPrimaryKeyColumns(configID: configID, table: table)

        return await TableProperties(
            table: tableInfo,
            stats: stats,
            columns: columns,
            indexes: indexes,
    ...
    public func fetchRoutineSource(configID: String, schema: String, name: String, type: RoutineType) async -> String {
        guard let (provider, handle) = ddlProvider(for: configID) else { return "" }
        return (try? await provider.fetchRoutineSource(handle: handle, schema: schema, name: name, type: type)) ?? ""
    }

    public func updateRoutine(configID: String, schema: String, name: String, type: RoutineType, source: String) async throws {
        guard let (provider, handle) = ddlProvider(for: configID) else { throw DatabaseConnectionError.notConnected }
        try await provider.updateRoutine(handle: handle, schema: schema, name: name, type: type, source: source)
        schemaVersion += 1
    }

    // MARK: - Dependencies

    public func fetchDependencies(configID: String, schema: String, table: String) async -> [String] {
        guard let (provider, handle) = ddlProvider(for: configID) else { return [] }
        return (try? await provider.fetchDependencies(handle: handle, schema: schema, table: table)) ?? []
    }

    // MARK: - Table Operations

    public func renameTable(configID: String, table: TableID, newName: String) async throws {
        guard let (provider, handle) = ddlProvider(for: configID) else { throw DatabaseConnectionError.notConnected }
        try await provider.renameTable(handle: handle, table: table, newName: newName)
        schemaVersion += 1
    }

    public func moveTable(configID: String, table: TableID, newSchema: String) async throws {
        guard let (provider, handle) = ddlProvider(for: configID) else { throw DatabaseConnectionError.notConnected }
        try await provider.moveTable(handle: handle, table: table, newSchema: newSchema)
        schemaVersion += 1
    }

    public func copyTable(configID: String, table: TableID, newName: String, copyData: Bool) async throws {
        guard let (provider, handle) = ddlProvider(for: configID) else { throw DatabaseConnectionError.notConnected }
        try await provider.copyTable(handle: handle, table: table, newName: newName, copyData: copyData)
        schemaVersion += 1
    }

    // MARK: - Transactions

    @Published public var transactionStates: [String: Bool] = [:]

    public func beginTransaction(configID: String) async throws {
        guard let (provider, handle) = ddlProvider(for: configID) else { throw DatabaseConnectionError.notConnected }
        try await provider.beginTransaction(handle: handle)
        transactionStates[configID] = true
    }

    public func commitTransaction(configID: String) async throws {
        guard let (provider, handle) = ddlProvider(for: configID) else { throw DatabaseConnectionError.notConnected }
        try await provider.commitTransaction(handle: handle)
        transactionStates[configID] = false
    }

    public func rollbackTransaction(configID: String) async throws {
        guard let (provider, handle) = ddlProvider(for: configID) else { throw DatabaseConnectionError.notConnected }
        try await provider.rollbackTransaction(handle: handle)
        transactionStates[configID] = false
    }

    public func isInTransaction(configID: String) -> Bool {
        transactionStates[configID] ?? false
    }

    // MARK: - Multi-statement Execution

    public func splitStatements(_ sql: String) -> [String] {
...
public enum DatabaseConnectionError: LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .timeout(let s): return "Query timed out after \(s)s"
        }
    }
}
