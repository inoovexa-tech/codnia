...
open class TerminalView: NSView, NSTextInputClient, NSUserInterfaceValidations, TerminalDelegate {
    ...
    private func scrollPrecise(with event: NSEvent) {
        ...
        preciseScrollAccumulator = preciseScrollAccumulator.truncatingRemainder(dividingBy: rowHeight)
        if ended && abs(preciseScrollAccumulator) < 1 {
            preciseScrollAccumulator = 0
        }
        if lines > 0 {
